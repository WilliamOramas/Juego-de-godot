...
var _fallback_qa: Dictionary = {}
var _is_fallback_mode: bool = false
...
func show_dialog(npc_name: String, lines: Array[String], ai_system_prompt: String = "", fallback_qa: Dictionary = {}) -> void:
	if is_open or lines.is_empty():
		return

	_current_ai_prompt = ai_system_prompt
	_fallback_qa = fallback_qa
	...
	if _fallback_qa.is_empty():
		badge_container.visible = true
	else:
		badge_container.visible = false # Se mostrarán solo si entramos en fallback mode
...
func _set_input_state(enabled: bool) -> void:
	...
	if enabled and input_line.is_inside_tree(): input_line.grab_focus()
...
func _start_typewriter() -> void:
	...
	if type_timer.is_inside_tree():
		type_timer.start(type_speed)
	else:
		# Modo testing sin arbol
...
func _on_badge_pressed(text: String) -> void:
	if _is_fallback_mode:
		_submit_fallback_message(text)
	elif input_line.editable:
		_submit_player_message(text)

func _submit_fallback_message(question: String) -> void:
	audio_select.play()
	var answer = _fallback_qa.get(question, "Lo siento, no sé la respuesta a eso.")
	_update_dialog_lines([answer])
...
func _on_ai_error_received(error: String) -> void:
	if is_open:
		if not _fallback_qa.is_empty() and not _is_fallback_mode:
			_enter_fallback_mode(error)
		else:
			_update_dialog_lines(["[Error de sistema: " + error + "]"])

func _enter_fallback_mode(_error: String) -> void:
	...
	# Create new badges for fallback
	for question in _fallback_qa.keys():
		var btn = Button.new()
		btn.text = str(question)
		btn.pressed.connect(func(): _on_badge_pressed(btn.text))
		badge_container.add_child(btn)
		
	badge_container.visible = true
	# Ya no llamamos a _update_dialog_lines para mantener el mensaje original en pantalla
