...
func _load_quest_data(quest_id: String) -> Variant:
	var path := "res://src/features/quests/%s.tres" % quest_id
...
func _process_notif_queue() -> void:
	...
	var notif := preload("res://src/features/menu/features/journal/quest_notification.tscn").instantiate() as QuestNotification
	if not notif:
		_notif_busy = false
		return
		
	notif.tree_exited.connect(_on_notif_exited)
	parent.add_child(notif)
	notif.setup(entry.text, entry.color)

func _on_notif_exited() -> void:
	_notif_busy = false
	call_deferred("_process_notif_queue")
